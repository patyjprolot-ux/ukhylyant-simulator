...
const RESOURCES = [
    ...
    // (MARKET_ASSETS бере тір<=3 БЕЗ tradeLock:'quest' — Р17, TRADE LOCK).
    ...
    // tradeLock: 'limited' (Р17, PATCH_2.0_CLAUDE_DECISIONS.md, 2026-08-16) —
    // модель TRADE LOCK. Уламки МОЖНА продати (лишок не мертвий вантаж), але НЕ
    // купити — інакше "безкоштовний шлях до донатних ящиків" перестає бути
    // безкоштовним: досить купити уламки за ТК замість того, щоб їх нафармити.
    { id: 'shard', name: 'Уламок пломби', emoji: '🧩', tier: 3, sell: 1600, tradeLock: 'limited' },
    ...
    { id: 'shard_starter', name: 'Уламок стартового набору', emoji: '🧩', tier: 3, sell: 2500, tradeLock: 'limited' },
    { id: 'shard_elite', name: 'Уламок елітного контейнера', emoji: '🧩', tier: 3, sell: 2500, tradeLock: 'limited' },
    { id: 'shard_wardrobe', name: 'Уламок модної валізи', emoji: '🧩', tier: 3, sell: 2500, tradeLock: 'limited' },
    { id: 'shard_legendary', name: 'Уламок легендарного схрону', emoji: '🧩', tier: 3, sell: 2500, tradeLock: 'limited' },
    ...
    // tradeLock: 'quest' (Р17) — прогресивний ключ, не товар: якщо купується за
    // ТК, переїзд на схрон 5 більше нічого не вартий, окрім грошей.
    // lore — сюжетна легенда ресурсу (ChatGPT, 2026-08-16). Поки ніде не рендериться:
    // прогрес-бар крафт-проєкту "route" живе в routes/sprints.js за флагом SPRINTS_V2
    // (вимкнено), тому екран, де це доречно показати, ще не активний для гравців.
    // Коли Спринти вмикаються — вивести цей текст біля routeProgress-бару.
    { id: 'route', name: 'Маршрут через кордон', emoji: '🗺️', tier: 3, sell: 2200, tradeLock: 'quest',
      lore: 'Кажуть, цю схему намалювали на звороті старого туристичного буклету. Щоб її роздобути, доводиться не лише збирати цеглу й дизпаливо, а й розгадувати натяки старих бабусь-слідопитів. Та щойно "Маршрут" потрапляє до рук — будь-який кордон стає трохи м\'якшим для тих, хто його знайшов.' },
    // Медична гілка (Р18 v3): направлення відкриває МОЖЛИВІСТЬ почати нову
    // хворобу (/api/disease/start). Кап зберігання = 1 — контролюється в
    // routes/hospital.js (не тут), щоб не можна було припасти наперед і
    // почати кілька хвороб одразу.
    { id: 'referral', name: 'Направлення до лікаря', emoji: '📝', tier: 3, sell: 0, tradeLock: 'quest' },
    // tradeLock: 'quest' — явно, а не лише "тір 4 не потрапляє у фільтр" (як
    // раніше): Р17 хоче, щоб застава читалась із самих даних, не з побічного
    // ефекту фільтра tier<=3.
    { id: 'ticket', name: 'Білий квиток', emoji: '🎫', tier: 4, sell: 9000, tradeLock: 'quest' },
    ...
    // tradeLock: 'quest' (Р17, 2026-08-16) — ПЕРЕГЛЯНУТО рішення. Раніше тут стояло
    // "навмисно торгується" (коли ресурси Спринтів щойно додавались, без моделі
    // TRADE LOCK). DeepSeek+ГПТ+твоя власна скарга на біржу дали конкретніше,
    // краще обґрунтоване рішення: якщо script/intel_data купуються за ТК, увесь
    // цикл Спринтів (роби контракт → отримай ресурс) стає необов'язковим —
    // гроші не мають дозволяти обійти геймплей.
    { id: 'script', name: 'Фрагмент коду', emoji: '💾', tier: 3, sell: 2500, tradeLock: 'quest' },
    { id: 'intel_data', name: 'Розвідані дані', emoji: '🛰️', tier: 3, sell: 12000, tradeLock: 'quest' },
    // Крипто-ключ має здобуватись контрактом, а не докуповуватись за ТК.
    { id: 'crypto_key', name: 'Крипто-ключ', emoji: '🔑', tier: 4, sell: 45000, tradeLock: 'quest' },
];
