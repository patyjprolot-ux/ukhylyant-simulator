...
const RECIPES = [
    ...
    // Портфоліо предметів (аудит 2026-08-13): coffee/coal падали з ящиків, але не
    // мали ЖОДНОГО застосування — чисте vendor trash. Простий ранній рецепт, щоб
    // вони мали сенс замість "продай і забудь".
    {
        id: 'hot_meal', name: 'Гарячий обід на буржуйці', emoji: '🍲',
        cost: { coffee: 8, coal: 5 },
        desc: '+15 до максимальної енергії (назавжди)',
        effect: { type: 'maxEnergy', amount: 15 },
    },
    {
    ...
    },
    {
        ...
        // Повторний аудит (2026-08-13): цінник fuel/sim/cash піднявся ОКРЕМО від цієї
        // правки, і кошт інгредієнтів виріс до ~10 110₴ — рецепт став ЗБИТКОВИМ
        // (−1 110₴ за крафт). Піднято до 15 000, щоб повернути цільову маржу ×1.48.
        id: 'smuggle_kit', name: 'Набір контрабандиста', emoji: '🎒',
        cost: { fuel: 12, sim: 8, cash: 3 },
        desc: 'Продається перекупу за 15 000 ТК',
        effect: { type: 'coins', amount: 15000 },
    },
    {
    ...
    // Довгий безкоштовний шлях до платних ящиків. 2026-08-13: кожен донатний ящик
    // тепер клеїться зі СВОГО окремого уламка (shard_starter/elite/wardrobe/
    // legendary, catalog/resources.js), який випадає бонус-роллом ~0.05% лише з
    // відповідного безкоштовного ящика (rollCrate, server.js) — набагато рідше,
    // ніж старий спільний 'shard' (0.8-4.7%). Тому й кількості на рецепт знижені
    // на порядок проти старих 10/25/45/75 — інакше шлях став би математично
    // недосяжним, а не просто довгим.
    {
        id: 'glue_starter', name: 'Склеїти Стартовий пакет', emoji: '🥡',
        cost: { shard_starter: 3, tape: 5 },
    ...
    },
    {
        ...
        cost: { shard_elite: 5, tape: 10, cash: 3 },
    ...
    },
    {
        ...
        cost: { shard_wardrobe: 8, stamp: 3, cash: 6 },
    ...
    },
    {
        ...
        cost: { shard_legendary: 12, stamp: 8, cash: 15, phone: 1 },
