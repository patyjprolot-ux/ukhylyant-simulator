...
const ECONOMY = {
    ...
    // Ремап бустерів під Спринти (Р4, PATCH_2.0_CLAUDE_DECISIONS.md, 2026-08-16).
    // clickVal/passive лишаються як є (боси й далі клікерні, пасив і далі тіка у
    // фоні завжди) — це ДОДАТКОВИЙ ефект тих самих рівнів, щоб hat/thermos не
    // ставали мертвими цифрами на схронах 2-8, де клікати вже нічим.
    // hat+thermos → Focus (менше вигорання за тап у Спринтах).
    UPGRADE_FOCUS_PER_LEVEL: 0.01,
    UPGRADE_FOCUS_CAP: 3,
    // jam+generator → швидкість відновлення вигорання (той самий дух, що дав
    // Р4: "пасив і так неконтрольовано розігнав економіку, переказ у 'швидше
    // готовий до наступного спринту' зберігає ідею 'менше простою', не гроші
    // нізвідки").
    UPGRADE_BURNOUT_DECAY_PER_LEVEL: 0.01,
    UPGRADE_BURNOUT_DECAY_CAP: 2,
    // 2026-08-09: 120₴ за повний реген енергії було копійками (менше за один
    // рівень Капелюха), повністю знецінювало "енергія — головний обмежувач
    // темпу". Піднято в 5 разів.
    ENERGY_DRINK_PRICE: 600,
    // Не більше 2 покупок енергетика за 5 хв (2026-08-09) — інакше ціна сама
    // по собі не заважає просто спамити відновлення й обходити ліміт енергії.
    ENERGY_DRINK_MAX_PER_WINDOW: 2,
    ENERGY_DRINK_WINDOW_MS: 5 * 60 * 1000,
    ...
    // Р18 v3 (2026-08-16): легалізація тепер ВІДКРИВАЄ схрон 8, а не навпаки —
    // максимальний рівень БЕЗ довідки легалізації = 7. /api/location/buy рівня
    // 8 (routes/economy.js) додатково перевіряє user.prestigeCount >= 1.
    PRESTIGE_UNLOCK_LEVEL: 7,
    ...
    // --- Медична гілка (PATCH 2.0 Р18 v3, 2026-08-16) — окрема сюжетна ---
    // вкладка, НЕ реакція на повістку (початкова версія з QTE-тапом на
    // повістці була архітектурною помилкою, відкочено — див.
    // PATCH_2.0_MEDICAL_QUESTLINE.md). Вилазка "Лікарня" незалежна від
    // інших локацій, не дає прокачувальних ресурсів. Здача аналізів —
    // окремий платний QTE (свій рушій, не спільний з медкомом-довідкою).
    HOSPITAL_MINUTES: 240,
    DISEASE_ANALYSIS_ROUNDS: 3,
    DISEASE_ANALYSIS_HITS_NEEDED: 2,
    DISEASE_QTE_WINDOW_MS: 1800, // фіксоване, не залежить від heat — це не про розшук
