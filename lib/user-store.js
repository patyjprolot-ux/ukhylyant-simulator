...
function createFreshUser(id, name) {
    return {
        ...
        // Компаньйони (редизайн 2026-08-21) — які companionQuest уже виконано
        // (petId -> true), тобто /api/pet/buy для цього petId розблоковано.
        companionUnlocked: {},
        // Маршрути втечі (Р15, 2026-08-21) — ендгейм-луп після легалізації.
        // Один активний забіг за раз (як старі вилазки до "Другої нори"), окрема
        // косметична валюта, не чіпає balance/resources.
        contraband: 0,
        smugglingRun: null,
        dailyTradeVolume: 0,
        dailyBribes: 0,
...
function migrateUser(user) {
    ...
    if (typeof user.companionUnlocked !== 'object' || user.companionUnlocked === null) user.companionUnlocked = {};
    if (typeof user.contraband !== 'number') user.contraband = 0;
    if (user.smugglingRun === undefined) user.smugglingRun = null;
