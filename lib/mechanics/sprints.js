...
const {
    ...
    QTE_BASE_MS, QTE_MAX_MS, QTE_MISS_PENALTY,
...
function burnoutPerTap(focusStat) {
    return BURNOUT_PER_TAP / Math.max(1, Number(focusStat) || 1);
}
...
function decayBurnout(user) {
    const now = Date.now();
    const last = user.lastBurnoutDecay || now;
    const stepMs = BURNOUT_DECAY_SEC * 1000;
...
function sprintSnapshot(user) {
    ...
    return {
        ...
        routeProgress: user.routeProgress || 0,
